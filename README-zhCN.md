...
身为 Ruby 开发者，有件事儿总令我心烦—— Python 开发者们有一个非常好的编程规范（[PEP-8][]），而我们却一直没有一个这样来自官方的、指导 Ruby 编码风格及最佳实践的指南性文档。我知道指南事关重大，但我也相信像 Ruby 这样的黑客社区有能力自己编写这梦寐以求的文档。

这份指南开始是作为我们公司内部的 Ruby 编程指南（实际上是由我所写的）而制定的。某天，我突然意识到，这个世界并不会因为一份公司内部的指南而增色，但是如果我把它贡献给社区，也许 Ruby 社区会对我所编写的指南感兴趣——那么，由社区驱动和制定的一份关于最佳实践、习惯用法和编码风格的 Ruby 编程指南必定会对世界有益。

从编写这份指南开始，我收到了来自世界各地优秀 Ruby 社区的很多用户反馈。衷心感谢所有建议及帮助！同心协力，我们能创造出让每一个 Ruby 开发者受益的资源。

另外，如果你对 Rails 感兴趣，你可以看看这份 [Ruby on Rails 编程指南][rails-style-guide]。

# Ruby 编程指南

这份 Ruby 编程指南推荐的是 Ruby 的最佳实践，Ruby 程序员据此可以写出可维护的高质量代码。我们只介绍实际使用中的用法——如果指南里面设计的用法过于理想化结果大家都拒绝使用或者根本没人用，那也就失去了指南的意义。

本指南分为几个小节，每一小节由几条相关的规则构成。我尽力在每条规则后面说明理由（如果省略了说明，那是因为其理由显而易见）。

这些规则不是我凭空想象出来的——它们中的绝大部分来自我多年以来作为职业软件工程师的经验，来自 Ruby 社区成员的反馈和建议，以及几个评价极高的 Ruby 编程资源，像[《Programming Ruby》][pickaxe]以及[《The Ruby Programming Language》][trpl]。
